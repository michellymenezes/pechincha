...
public class HerokuGetProductsByCategoryTask extends AsyncTask {
    ...
    public HerokuGetProductsByCategoryTask(String url, ProductListener listener) {
        endpoint = url;
    ...
    protected Boolean doInBackground(Object... params) {
        ...
        try {
                ...
                Log.d(TAG, String.valueOf(productsJSON));
                    ...
                    for (int i = 0; i < productsJSON.length(); i++) {
                        if ( productsJSON.getJSONObject(i).length() > 9){
                            ...
                            double size = productsJSON.getJSONObject(i).getDouble("size");
                            String sizeUnity = productsJSON.getJSONObject(i).getString("sizeUnity");
                            Product product = new Product(id,name,brand,descripition,image,code,category,subcategory, size, sizeUnity);
            ...
            e.printStackTrace();
            Log.v("ERRO1", e.toString());

            return false;
        } catch (IOException e) {
            e.printStackTrace();
            Log.v("ERRO2", e.toString());
    ...
    protected void onPostExecute(Object o) {
        ...
        if (products != null) {
            mListener.OnGetProductsByCategoryReady(true, this.products);
        } else {
            mListener.OnGetProductsByCategoryReady(false, this.products);
        }
    }
